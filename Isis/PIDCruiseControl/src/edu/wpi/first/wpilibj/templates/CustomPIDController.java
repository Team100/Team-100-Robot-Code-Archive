...
//        }
    ...
    private void calculate() {
        ...
        if (enabled) {
            currInput = pidInput.pidGet() / m_gearRatio;
            double result;
            ...
            SmartDashboard.putDouble("Filtered_Position", m_filteredPosition);
            
            currInstVeloc = currInput - prevInput;
                currDeriv = currInstVeloc - prevInstVeloc;
            if ((prevDeriv > 0.0 && currDeriv < 0.0) || (prevDeriv < 0.0 && currDeriv > 0.0)) {
                numOscills++;
            }
            if (currInstVeloc > prevInstVeloc && currInstVeloc > m_maxVeloc) {
                m_maxVeloc = currInstVeloc;
            }
            SmartDashboard.putDouble("numOscills", numOscills);
            SmartDashboard.putDouble("Diff_btwn_actual_&_goal_veloc", m_maxVeloc - m_velocSetpoint);

            synchronized (this) {
                m_error = m_filteredPosition - currInput;
                prevInput = currInput;
...
//                }
                ...
                //algorithm!
                m_dTerm = (m_velocSetpoint - (m_error - m_prevError) / kDefaultPeriod);
                m_result = (m_P * m_error + m_I * m_totalError - m_D * m_dTerm);
                //m_result += (m_filteredPosition * m_F);
                m_prevError = m_error;

                SmartDashboard.putDouble("m_dTerm", m_dTerm);
